...
public class AugmentLoanKafkaStream {
    ...
    @Autowired
    public AugmentLoanKafkaStream(@Value("${spring.application.name}") String applicationName,
    ...
    public void startStream() {
        ...
        streamBuilder.stream(topics.getAugmentLoan(), kafkaHelper.consumedWith(AugmentLoanEvent.class))
                .foreach(this::save);
    ...
    private void save(String key, AugmentLoanEvent event) {
        ...
        LOGGER.info("Received event with key {} on topic {}", key, topics.getAugmentLoan());
    ...
    private Properties properties() {
        AcmeProperties.Kafka.Topics topics = acmeProperties.getKafka().getTopics();
        return kafkaHelper.properties(applicationName.concat("-").concat(topics.getAugmentLoan()));
    }
